...
public class OldCustomer extends Customer {
    ...
    public String payBill(String meterCode, int paymentAmount) {
        ...
        if (paymentAmount < balanceDue) {
             // As per original logic, payment must be >= balanceDue. If partial payment is allowed, this logic needs change.
            // For now, sticking to original logic: paymentAmount >= balanceDue
            // The original code had a confusing message here. Assuming it meant paymentAmount should not be less than balanceDue.
            // If paymentAmount is less than balanceDue, it was an error.
            // Let's adjust to allow paying *at least* the balance due, or an exact amount.
            // The original code implies paymentAmount must be >= balanceDue for success.
            // If paymentAmount < balanceDue, it was an error. Let's re-evaluate this logic for GUI.
            // For now, let's assume the user wants to pay an amount, and it should cover the bill or be rejected.
            // The original code was: if (paymentAmount >= balanceDue) { success } else { error: shouldn't be less than balance due }
            // This implies paymentAmount *must* be >= balanceDue.
            return "Error: Payment amount (" + paymentAmount + ") is less than the balance due (" + balanceDue + "). Please pay the full amount or more.";
        }

        // If paymentAmount >= balanceDue
        int change = paymentAmount - balanceDue;
        balanceDue = 0;
        unpaidMonths = 0;
        // FileSystem.saveCustomerData will be called by the GUI controller
        return "Payment of " + paymentAmount + " successful. Change: " + change + ". Remaining balance: 0.";
    }
